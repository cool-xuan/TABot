...
def check_tensor(tensor, stage=''):
    contains_nan = torch.isnan(tensor).any().item()
    contains_inf = torch.isinf(tensor).any().item()
    contains_neg = torch.lt(tensor, 0).any().item()

    if contains_nan:
        print(f"In {stage}, Tensor contains NaN")
    if contains_inf:
...
class TABOTLlamaModel(LlamaModel):
    ...
    def init_video_Qformer(cls, num_query_token, vision_width,num_hidden_layers =2):
        encoder_config = BertConfig.from_pretrained("./ckpt/blip2")
    ...
    def forward(
                ...
                if vision_tower.config.use_im_start_end:
                    ...
                    if orig_embeds_params is not None:
                        ...
                        cur_new_input_embeds = torch.cat(
                            ...
                            dim=0)
                    new_input_embeds.append(cur_new_input_embeds)
                check_tensor(cur_new_input_embeds, 'Image Encoder')
        ...
        if videos is not None:
            ...
            if type(videos) is list:
                ...
                for video in videos:
                    if video is None:
                        ...
                        check_tensor(video, 'Video tensor')
                        video_embed, _ = self.encode_videoQformer_visual(video.unsqueeze(0))
                    check_tensor(video_embed, 'Video Qformer')
            ...
            for cur_input_ids, cur_input_embeds, cur_video_embeds in zip(input_ids, inputs_embeds, video_embeds):
                check_tensor(cur_input_embeds, 'cur_input_embeds')
                ...
                for video_start_token_pos, cur_video_embed in zip(video_start_tokens, cur_video_embeds):
                    check_tensor(video_start_token_pos, 'video_start_token_pos')
                    check_tensor(cur_video_embed, 'cur_video_embed 1')
                    cur_video_embed = cur_video_embed.to(device=cur_input_embeds.device)
                    check_tensor(cur_video_embed, 'cur_video_embed to device')
                    ...
                    check_tensor(cur_new_input_embeds, 'Video Encoder')
                new_input_embeds.append(cur_new_input_embeds)
            inputs_embeds = torch.stack(new_input_embeds, dim=0)

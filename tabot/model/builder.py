...
def load_pretrained_model(model_path, model_base=None, load_8bit=False, load_4bit=False, device_map="auto"):
    ...
    image_processor = CLIPImageProcessor.from_pretrained('./ckpt/clip-vit-large-patch14-336' ,torch_dtype=torch.bfloat16)
    image_tower = CLIPVisionModel.from_pretrained('./ckpt/clip-vit-large-patch14-336',torch_dtype=torch.bfloat16,low_cpu_mem_usage=True).to(CONFIG.device)
